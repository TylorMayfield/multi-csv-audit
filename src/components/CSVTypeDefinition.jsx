...
export default function CSVTypeDefinition() {
  ...
  const handleDeleteType = async (id) => {
    try {
      const response = await fetch(`/api/csv-types/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        ...
        <div className="space-y-4">
          {types.map((type) => (
            <div key={type.id} className="border rounded-lg p-4">
              ...
              <button onClick={() => handleEditType(type.id)} className="btn btn-secondary">
                <PlusIcon className="h-4 w-4" />
              </button>
              <button onClick={() => handleDeleteType(type.id)} className="btn btn-danger">
                <TrashIcon className="h-4 w-4" />
              </button>
            </div>
          ))}
